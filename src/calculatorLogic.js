...
//THE BELOW IS FROM MICHAEL JACKSON'S YOUTUBE TUTORIAL: https://www.youtube.com/watch?v=ZtU7Mhf9vN8
    ...
    inputDigit(digit){
      ...
      if(signOfOperation === "="){this.setState({
  ...
  operations(sign){
    ...
    if(savedValue === null){this.setState({
    ...
    //  if (savedValue.charAt(savedValue.length - 1) === "+" || "-" || "/" || "*"){
        this.setState({displayValue: eval(savedValue + displayValue), savedValue: eval(savedValue + displayValue)})
      ...
      <div className="calculator">
        ...
        <div className="calculator-keypad">
          ...
          <div className="operator-keys">
            <button className="calculator-key key-divide" onClick={() => this.operations("/")}>÷</button>
            <button className="calculator-key key-multiply" onClick={() => this.operations("*")}>×</button>
            <button className="calculator-key key-subtract" onClick={() => this.operations("-")}>-</button>
            <button className="calculator-key key-add" onClick={() => this.operations("+")}>+</button>
            <button className="calculator-key key-equals" onClick={() => this.equals()}>=</button>
          </div>
        </div>
      </div>
  )
